	...
	if ($result->num_rows > 0) {
		...
		if ($info['admin'] == 1) {
			...
			if (isset($_POST['foodname'])) {
				...
				$sql = "INSERT INTO $table_sales(sale_user_name,sale_food_id,sale_food_name,sale_food_num,sale_food_cost,sale_state,sale_date) VALUES('$username','$food_id','$foodname','$foodnum','$foodcost','0','$time')";
				$result = $mysqli->query($sql) or die($mysqli->error);
				if ($result) {
					echo "成功加入购物车！现在您可以在<a href='user/show_sale.php'>历史订单</a>中查看";
				}
			}
		}
	}
